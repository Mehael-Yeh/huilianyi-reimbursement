...
面向 Codex Agent 的汇联易 A2 API 草稿填报工具。项目覆盖历史校准、票据识别与分类、差旅申请、差旅报销、个人报销、费用保存、回读验收和 Excel 核对表交付。

## 核心边界

- 只创建或编辑草稿，不提交、删除、关闭或撤回单据。
- 差旅报销必须关联已审核且未关闭的差旅申请。
- 个人报销不关联申请单。
- OID、表单和费用类型从当前租户动态获取。
- 凭据只通过交互输入或环境变量传递，不写入仓库。
- 仅上传 PDF、OFD、ZIP、XML；图片票据不上传。
