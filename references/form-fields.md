...
- `入住城市`（LOCATION）：从酒店销售方名称/地址推断；多地 `showValue` 用中文逗号连接。
- `开始结束日期`（START_DATE_AND_END_DATE）：JSON 中写整张差旅报销的起止日期和含首尾的天数。
- `客户名称`：不可靠时留给用户补录，不填写占位文案。

过路费的通行单/汇总单写入费用顶层 `attachments`，不计入 `receiptList`，不重复计金额。
