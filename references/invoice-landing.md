...
- 汇联易票据 DTO 的 `totalAmount`、`unUsedAmount` 等字段可能以分为单位；存在票据 DTO 标志时换算为元，不能把整数分值直接当作元。
- 创建费用前核对剩余可报销金额；余额为零通常表示票据已被其他单据锁定或报销，应停止并说明，不继续调用最终保存接口。
...
出差补贴默认参考金额为补贴天数乘以 100，客户名称允许为空。该公式只作通用默认值；公司制度或用户确认的标准不同时不得阻止保存，应提示差异并采用实际标准。
