...
因此，事后补申请时应先完成已知票据分类，以差旅费用计划生成申请预算基线。不能把“一致”写成绝对规则：实际报销减少、补贴调整、停车费等意外费用或历史类别别名都可能造成差异，但必须逐类审计并说明。

申请类目是汇总预算池，不是发票槽位。一个类目可以包含多条预算行，也可以被多张同类发票共同使用。报销落账时：

- 找到申请中所有同类预算行，将其数值 `id` 作为 `applicationCustomBudgetId` 列表；
- 每张同类发票复用该列表，形成多张发票到同类预算项目的汇总关系；
- `budgetOID` 不是此接口的 ID，传入会校验失败；标量 ID 也会失败；
- 申请缺少的实际类别使用空列表，通过手录费用按真实类别增补；
- 金额差额是审计信息，不是禁止报销的条件。
